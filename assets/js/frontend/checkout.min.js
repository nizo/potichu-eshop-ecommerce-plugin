	...
	*/
	...
	.submit( function() {
		...
		if ( $form.triggerHandler( 'checkout_place_order' ) !== false && $form.triggerHandler( 'checkout_place_order_' + $( '#order_review input[name=payment_method]:checked' ).val() ) !== false ) {
			...
			if (!jQuery('#billing_firm_data_region_field input').is(':checked')) {
				processed_data = $form.find(':not(#billing_firm_data_region_field input)').serialize();
			} else {
				processed_data = $form.serialize()
			}
