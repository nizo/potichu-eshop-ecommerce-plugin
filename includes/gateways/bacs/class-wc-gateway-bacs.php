...
class WC_Gateway_BACS extends WC_Payment_Gateway {
    ...
    private function bank_details( $order_id = '' ) {
    	...
    	echo '<h3>' . __( 'Our Bank Details', 'woocommerce' ) . '</h3>' . PHP_EOL;
    	...
    	if ( ! empty( $bacs_accounts ) ) {
	    	foreach ( $bacs_accounts as $bacs_account ) {
				...
				if ( $bacs_account->account_name || $bacs_account->bank_name ) {
					...
					echo '<li>Účet:<strong>' . implode( ' - ', array_filter( array( $bacs_account->account_name, $bacs_account->bank_name ) ) ) . '</strong></li>' . PHP_EOL;
				}	  
				...
				echo '<li class="' . esc_attr( 'vs' ) . '">' . esc_attr( 'Variabilný symbol' ) . ': <strong>' . wptexturize( $order_id ) . '</strong></li>' . PHP_EOL;
    ...
    public function process_payment( $order_id ) {
		...
		//wp_update_post( array( 'ID' => $this->id, 'post_status' => 'wc-processing' ) );
		//$this->post_status = 'wc-processing';
