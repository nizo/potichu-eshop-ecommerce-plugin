...
if ( ! function_exists( 'woocommerce_taxonomy_archive_description_with_image' ) ) {

	/**
	 * Show an archive description on taxonomy archives with image
	 *
	 * @access public
	 * @subpackage	Archives
	 * @return void
	 */
	function woocommerce_taxonomy_archive_description_with_image() {
		if ( is_tax( array( 'product_cat', 'product_tag' ) ) && get_query_var( 'paged' ) == 0 ) {
			$description = wpautop( do_shortcode( term_description() ) );
			if ( $description ) {
				...
				//echo '<div class="thumbnail"><img width="180" height="180" src="' . $cat_thumb_url . '"/ ></div>';				
				echo '<div class="thumbnail" style="background-image: url(\'' . $cat_thumb_url . '\');"></div>';				
				echo '<div class="meta"><h3 class="title">' . single_term_title('', false) . '</h3>';
				echo '<span class="description">' . wp_strip_all_tags($description) . '</span></div>';
	...
	function woocommerce_product_archive_description() {
		if ( is_post_type_archive( 'product' ) && get_query_var( 'paged' ) == 0 ) {
			...
			if ( $shop_page ) {
				...
				if ( $description ) {
					echo '<div class="page-description">' . $description . '</div><div style="clear:both;"></div>';
				}
			}
		}
	}
...
if ( ! function_exists( 'woocommerce_slider_and_categories' ) ) {
	
	function woocommerce_slider_and_categories() {
		if (is_front_page()) {			
			...
			foreach( $prod_categories as $prod_cat ) :
				...
				echo '<a class="category-link" href="'. get_term_link( $prod_cat, 'product_cat' )  .'">';
...
if ( ! function_exists( 'woocommerce_cart_totals_netto' ) ) {
	
	function woocommerce_cart_totals_netto() {
		wc_get_template( 'cart/cart-totals-netto.php' );
	}
}
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		case "country" :
			...
			case "region" :
				...
				$field .= potichu_generate_regions_options();;
		...
		case "select" :
			...
			if ( ! empty( $args['options'] ) )
				...
				$field .= '<label for="' . esc_attr( $args['id'] ) . '" class="' . esc_attr( implode( ' ', $args['label_class'] ) ) .'">Okres' . $required . '</label>';
