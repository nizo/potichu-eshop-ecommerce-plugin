...
class WC_Structured_Data {
	...
	function is_shop() {
		return ( is_post_type_archive( 'product' ) || is_page( 
			( 'shop' ) ) );
	}	
	...
	public function output_structured_data() {		
		...
		if ( $data = $this->get_structured_data( $types ) ) {
	...
	*/
	...
	public function generate_product_data( $product = null ) {
		...
		//var_dump($product->post->post_title);
		...
		$markup['description'] = $product->post->post_excerpt;
		...
		if ( '' !== $product->get_price() ) {
			if ( $product->is_type( 'variable' ) ) {
				...
				$markup_offer = array(
					...
					'price' => wc_format_decimal( $product->get_price_including_tax(), wc_get_price_decimals() ),
				);
			}
