...
class WC_Email_Customer_New_Account extends WC_Email {
	...
	function trigger( $user_id, $user_pass = '', $password_generated = false ) {
		...
		if ( $user_id ) {
			...
			$this->trigger_from_social = false;
		}
	...
	function trigger_from_social( $user_id, $user_pass = '') {
		...
		if ( $user_id ) {
			...
			$this->user_email         = stripslashes( $this->object->user_email );
			$this->recipient          = $this->user_email;
			$this->password_generated = true;
			$this->trigger_from_social = true;
		}
	...
	function get_content_html() {
		...
		wc_get_template( $this->template_html, array(
			...
			'social_network_register' => $this->trigger_from_social,
			'sent_to_admin' => false,
			'plain_text'    => false
		) );
		return ob_get_clean();
