...
/**
 * Customer Canceled Order Email
 *
 * Order complete emails are sent to the customer when the order is marked complete and usual indicates that the order has been shipped.
 *
 * @class 		WC_Email_Customer_Canceled_Order
 * @version		2.0.0
...
class WC_Email_Customer_Canceled_Order extends WC_Email {
	...
	function __construct() {
		...
		$this->title 			= __( 'Objednávka stornovaná', 'woocommerce' );
		...
		$this->heading 			= __( 'Your order is canceled', 'woocommerce' );
		$this->subject      	= __( 'Your {site_title} order from {order_date} is canceled', 'woocommerce' );

		$this->template_html 	= 'emails/customer-canceled-order-' . get_option('web_locale') . '.php';
