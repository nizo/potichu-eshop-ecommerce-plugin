...
class WC_Checkout {
	...
	public function process_checkout() {
		...
		$this->posted['validate_only'] = isset( $_POST['validate_only'] ) ? true : false;
		...
		if ($this->posted['shipping_region'])
			$_SESSION['shipping_region'] = $this->posted['shipping_region'];
		if ($this->posted['billing_region'])
		...
		if ($this->posted['validate_only']) {
			...
			$validationResult = ($messages == '') ? 'validated' : 'failure';
		...
		if ( ! empty( $_POST[ $input ] ) ) {
			...
			switch ( $input ) {
				case 'billing_firm_data_region' :
					return get_user_meta( $user_ID, 'billing_firm_data_region', true );
				case 'billing_company_name' :
					return get_user_meta( $user_ID, 'billing_company_name', true );
				case 'billing_ico' :
					return get_user_meta( $user_ID, 'billing_ico', true );
				case 'billing_dic' :
					return get_user_meta( $user_ID, 'billing_dic', true );
				case 'billing_ic_dph' :
					return get_user_meta( $user_ID, 'billing_ic_dph', true );
