...
class WC_AJAX {
	...
	public static function update_order_review() {
		...
		if (isset($_POST['shipping_region'])){
			...
			potichu_log('shipping region SET');
		}
		if (isset($_POST['billing_region'])) {
			$_SESSION['billing_region'] = $_POST['billing_region'];
			potichu_log('billing region SET');
		}
	

		WC()->cart->calculate_totals();

		potichu_log('after CALCULATE TOTALS');
	...
	/* AJAX update user region settings */
	 
	public function update_regions($data2) {

		$user_ID = get_current_user_id();

		potichu_save_user_region_field_checkout($user_ID);
		potichu_save_shipping_type($user_ID);		
				
		$data = array(			
			'billing' => $_POST['billing_region'],
			'shipping' => $_POST['shipping_region'],
			'shippingType' => $_POST['ship_to_different_address'],
			'userID' => $user_ID		
		);
	 ...
	 * Process ajax checkout form
	 */
	public static function validate_user_data($input) {		
