...
class WC_Product {
	...
	public function __get( $key ) {
		...
		if ( in_array( $key, array( 'downloadable', 'virtual', 'manage_stock', 'featured', 'sold_individually' ) ) ) {
		...
		} elseif ( 'backorders' == $key ) {
			$value = ( $value = get_post_meta( $this->id, '_backorders', true ) ) ? $value : 'notify';
	...
	public function get_availability() {
		...
		if ( $this->managing_stock() ) {
			...
			if ( $this->is_in_stock() && $this->get_total_stock() > get_option( 'woocommerce_notify_no_stock_amount' ) ) {
				...
				//$availability = 'Na externím sklade';
	 ...
	 * @return string
	 */
	public function get_price_including_tax( $qty = 1, $price = '', $forceFormat = false) {
